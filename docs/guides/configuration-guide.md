...
For copy-paste examples of every pattern, see [Examples](../examples/index.md). For the formal contract, see [Design Specification](../reference/record-health-check-design-spec.md). For setup walkthrough, see [Getting Started](../installation/getting-started.md). For action link examples, see [Action Links and Fix Instructions](action-links.md). For Show Troubleshooting Details, see [Show Troubleshooting Details](show-diagnostics.md).
...
| **Compare two queries** | `CompareTwoQueries` | Two independent SOQL results must be compared (single value or list). |
...
```text
PassConditionFormula__c: OR(NOT(ISBLANK(Phone)), NOT(ISBLANK(Website)))
FailureMessage__c: {!record.Name} needs either a Phone or Website.
```
...
```text
PassConditionFormula__c:      BLANKVALUE(Debit_Total__c, 0) = BLANKVALUE(Credit_Total__c, 0)
DisplayFoundFormula__c:    BLANKVALUE(Debit_Total__c, 0)
DisplayExpectedFormula__c: BLANKVALUE(Credit_Total__c, 0)
```
...
| -------------------- | ------------ | --- |
| A SOQL result vs a **fixed value** | Check records with a query | `ExpectedValueSource__c = FixedValue`, set `ExpectedFixedValue__c` |
| A SOQL result vs a **record formula** | Check records with a query | `ExpectedValueSource__c = RecordFormula`, set `ExpectedRecordFormula__c` |
| A SOQL result vs **another SOQL result** | Check records with a query / Compare two queries | `ExpectedValueSource__c = AnotherQuery` (or the Compare-two-queries method) |
| Two values **on the record** (formula vs formula, or formula vs fixed value) | Check fields on this record | Encode the comparison in `PassConditionFormula__c`; optionally add Found/Expected to display each side |
...
- `OneResult` + single-value operators for single values.
- `CompareAsLists` + `ListsOverlap`, `ListContainsAll`, or `ExactListMatch` for lists (**case-insensitive** list matching; `Contains` / `DoesNotContain` on single-value text are **case-sensitive**).
...
| Apex Settings (JSON) | `ApexParametersJson__c` | Optional tuning map passed as `context.parameters` |
...
- [ ] `NoRowsResult__c` is set for Any/All/CompareAsLists Rules.
...
- [ ] **Show Troubleshooting Details** is off for production unless actively troubleshooting (requires `Record_Health_Check_View_Details` via `Record_Health_Check_Admin`: see [Troubleshooting Details](show-diagnostics.md)).
...
| Null field on existing row (multi-row Query) | Rows returned but value null + `SkipRecordsWithMissingValue` → **SKIPPED** / `VALUE_IS_EMPTY` (not `NoRowsResult__c`) |
| CompareTwoQueries empty query side (OneResult) | Governed by **`NoRowsResult__c`** before null-field logic: distinct from null on a returned row |
...
| Empty multi-row query result | Defaults to `Skip` when `NoRowsResult__c` is blank |
