...
| Advanced: Formula Value Type | `ScalarFormulaReturnType__c` | Picklist | Yes | Optional type hint for formulas used as comparison values or list-check values. Leave `Auto` unless you know the formula returns a specific type. |
...
| Expected Value | `FixedValue__c` | Text | When source = FixedValue | Literal (for example, `0`, `Approved`). For numeric checks, this can be the threshold. |
