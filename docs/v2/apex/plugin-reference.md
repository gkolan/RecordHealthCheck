...
How a plugin class reads the record, parses JSON parameters, structures `evaluate`, and returns results. For the interface summary see [Apex plugin contract](plugin-contract.md). To call the engine from Apex or Flow without writing a plugin, see [Programmatic API and Flow](programmatic-api.md).
...
| ----------- | -------- | ------- |
| Apex Parameters (JSON) | `ApexParametersJson__c` | `{"daysBack": 90, "minScore": 80}` |
...
Walkthrough: [01: Recent activity](https://github.com/gkolan/RecordHealthCheck-Examples/blob/main/docs/pattern-library/apex/01-recent-activity.md).
