...
How a plugin class reads the record, parses JSON parameters, structures `evaluate`, and returns
results. For the end-to-end example see [Apex example](apex-example.md). To call the engine
without writing a plugin, see the [Apex API](public-api.md) or [Flow actions](../flow/actions.md).
...
Each Rule evaluation receives **one** `RecordHealthCheckContext`. The Rule metadata object is **not** passed into `evaluate`: tunable values belong in **Apex Parameters (JSON)** or as constants in Apex.

The evaluator resolves **Apex Class** with `Type.forName(ApexClass__c)`. An unpackaged class normally
uses its class name only. A class supplied by managed code may require the namespace-qualified type
name accepted by `Type.forName`; verify the exact resolvable name in the target org before activating
the Rule.
...
| ----------- | -------- | ------- |
| Apex Parameters (JSON) | [`ApexParametersJson__c`](../metadata/rule-fields.md#apex-parameters-json-apexparametersjson__c) | `{"daysBack": 90, "minScore": 80}` |
...
`expectedValueSource` with `RecordHealthCheckValueSource.Detail`. The engine converts those internal
details to `actualValueDetail` / `expectedValueDetail` only for users with
`Record_Health_Check_View_Details`.

The details never render on the Lightning card. They appear in the F12 `[RHC] Source detail`
console group only when **Show Diagnostics** is enabled for the Check Set. The permission controls
whether the strings exist; the flag controls whether the LWC emits them. See
[Show Diagnostics](../guides/show-diagnostics.md#what-you-see-in-the-browser-console).
...
Use the canonical [pipeline-readiness example](apex-example.md#example-account-pipeline-readiness).
...
- **Breaking changes:** Removing or renaming an interface member requires a documented breaking
  product release.
- **Deprecations:** None in V2.

## Related

| Doc | Purpose |
| --- | ------- |
| [Apex example](apex-example.md) | End-to-end starting point |
| [01: Recent activity](https://github.com/gkolan/RecordHealthCheck-Examples/blob/main/docs/pattern-library/apex/01-recent-activity.md) | Multi-object + JSON |
| [02: Opportunity health](https://github.com/gkolan/RecordHealthCheck-Examples/blob/main/docs/pattern-library/apex/02-open-opportunity-health.md) | Child loop + Found/Expected |
| [03: Strategic readiness](https://github.com/gkolan/RecordHealthCheck-Examples/blob/main/docs/pattern-library/apex/03-strategic-readiness.md) | Scoring + JSON |
