...
Machine-readable codes that explain why a check did not produce a normal `PASS` / `FAIL`, or why a card could not load. Integrations must key on the **code**, never on display text. Codes are additive `UPPER_SNAKE_CASE` strings.

> [!IMPORTANT]
> **Public vs diagnostics-only:** `FIELD_NOT_ACCESSIBLE` and `RECORD_NOT_ACCESSIBLE` never appear as the public `reasonCode` on a result. The engine remaps them to `CANNOT_EVALUATE`. When **Show Diagnostics** is on and the user has `Record_Health_Check_View_Details`, the specific code is available on `adminDetail.reasonCode`.
...
| `GOVERNOR_LIMIT_RISK` | `UNABLE_TO_EVALUATE` | Query would exceed safe row / governor budget. |
...
| `FORMULA_EVAL_LIMIT` | `UNABLE_TO_EVALUATE` | Transaction FormulaEval budget exhausted (framework stops before the platform hard limit). |
...
| Code | Typical status | Meaning |
| --- | --- | --- |
| `LEGACY_FLAT_TOKEN` | `UNABLE_TO_EVALUATE` | Legacy `{!Id}`-style token rejected under strict V2 namespaced syntax. <!-- legacy-token-ok --> |
...
| `TOKEN_LIMIT_EXCEEDED` | `UNABLE_TO_EVALUATE` | Too many tokens in one template. |
| `RESOLVED_TEMPLATE_TOO_LONG` | `UNABLE_TO_EVALUATE` | Resolved text exceeded the allowed length. |
...
2. Treat unknown future codes as additive — do not fail closed on “code not in my enum” unless you own a strict allowlist.
