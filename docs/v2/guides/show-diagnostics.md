...
Show Diagnostics shows extra technical detail on the health check card and in the browser console. It is for troubleshooting only: leave it **off** on production Check Sets when not actively investigating a problem.

Show Diagnostics does **not** save history and does **not** write any records: it only adds on-screen and console detail for the current run.
...
| ----------------- | --------------- |
| `Record_Health_Check_View_Details` | **Always** (even with Show Diagnostics off): Formula **Passes when** labels on entitled rows. **With Show Diagnostics on:** gray troubleshooting lines, **Troubleshooting detail** blocks, console footnote, the `[RHC]` console summary, and comparison diagnostic details notes (`actualValueDetail` / `expectedValueDetail`) inside the nested `[RHC] Source detail` group. |
...
| What | Description |
| ---- | ----------- |
| **Gray line under each result** | Compact summary, for example `FAIL · FORMULA_FALSE · 38ms · Formula`: status, reason code, time taken, Evaluation Type (API value) |
