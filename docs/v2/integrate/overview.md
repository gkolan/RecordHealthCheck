...
Run metadata-defined health checks from Lightning, Apex, or Flow, then optionally notify other
...
| --- | --- | --- | --- |
| Show health on a record page | [Lightning component](../lwc/runs-and-events.md) | Rows and Set summary | `USER_INITIATED`; automatic load is blocked |
| Make a code-level decision | [Apex API](../apex/public-api.md) | Typed Rule or Set response | `APEX_API`, `SCHEDULED`, or `BATCH` |
| Branch in automation without code | [Flow actions](../flow/actions.md) | Flow output variables and JSON | `FLOW` |
| React asynchronously or export results | [Platform events](../reference/lifecycle-events.md) | Event payload | Depends on the publisher |
| Add a custom evaluation algorithm | [Apex example](../apex/apex-example.md) | Normal Rule result | Inherits the calling run |
...
For method overloads, fields, limits, and exceptions, use the [Apex API reference](../apex/public-api.md).
...
Use stable statuses, reason codes, severities, and developer names for automation. Do not branch on
