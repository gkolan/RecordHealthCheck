...
> **On this page**
>
> Review the authoritative runtime architecture, metadata behavior, evaluator rules, UI decisions, limits, and known edge cases.

**Version:** 2.0.0 (2026-07-13) — aligned with the shipped Custom Metadata in `force-app/`
...
For Setup walkthroughs and field tables, see [Create your first Rule](../installation/03-create-your-first-rule.md) and the [Configuration Guide](../guides/configuration-guide.md). This document is the formal contract: runtime layout, evaluators, Reason Codes, and integration boundaries.

## Who should use this specification

Use this page when you are maintaining the Framework, reviewing an architectural change, or verifying behavior that must remain stable across Lightning, Apex, and Flow. It is intentionally more detailed than the task guides.

| Your question | Go to |
| --- | --- |
| How does a Lightning record page run and reveal Rules? | [Runtime architecture](#3-runtime-architecture) and [LWC behavior](#15-lwc-behavior) |
| Which Check Set or Rule fields affect runtime behavior? | [Check Set model](#4-check-set-model-record_health_check_set__mdt) and [Rule model](#5-rule-model-record_health_check_rule__mdt) |
| How does an Evaluation Type reach a result? | The evaluator sections for `FORMULA`, `QUERY`, `COMPARE_TWO_QUERIES`, and `APEX` |
| What security context applies to SOQL and record access? | The security and query-processing sections |
| What may an Apex, Flow, or lifecycle-event consumer rely on? | The public API, response, and event contract sections |

If your goal is to configure a Rule in Salesforce Setup, begin with the [Example library](../examples/README.md) instead.

> [!IMPORTANT]
> **Metadata vocabulary:** Custom Metadata stores picklist values as `UPPER_SNAKE_CASE` (for example `FORMULA`, `RUN_ON_REQUEST`, `SHOW_EACH_CHECK`, `CRITICAL`). Apex maps selected Check Set modes to legacy LWC DTO strings at the wire boundary (`RUN_ON_LOAD` → `Automatic`, `ONE_BY_ONE` → `OneAtATime`, `SHOW_COUNT_ONLY` → `Hide`, and so on). Author and validate metadata with the stored API values. Field Setup labels are listed in [Check Set fields](../metadata/fields-check-set.md) and [Rule fields](../metadata/fields-rule.md).
...
- [Create your first Rule](../installation/03-create-your-first-rule.md) → [Configuration Guide](../guides/configuration-guide.md): configure checks in Setup
- [Apex API](../integration/apex-api/public-api.md): call a Rule or Check Set from Apex
- [Flow actions](../integration/flow-actions.md): call a Rule or Check Set from Flow
- [Lightning component](../integration/lightning-component.md): understand Lightning publication
- [Lifecycle events](../integration/lifecycle-events.md): subscribe to lifecycle events
- [Reason Codes](reason-codes.md): look up a Status Reason Code
- [Architecture Map](architecture-map.md): navigate the source code
- Sections 1-3, 9-15, and the field reference in Sections 4-5 below: runtime layout and contracts
...
- **Evaluator**: the Apex code path for a Rule (`FORMULA`, `QUERY`, `COMPARE_TWO_QUERIES`, or `APEX`).
...
- **Background health monitoring as a product feature.** There is no built-in scheduler. Checks **can** be invoked via the public entry point (`RecordHealthCheck.runRule` / `runSet`) and the separate packaged Rule and Set Flow actions, subject to call caps (`MAX_RECORDS_PER_CALL` / `MAX_EVALUATIONS_PER_CALL`).
...
| `RecordHealthCheck` entry point | Apex entry for one Rule or one Check Set (single + bulk), with call caps and opt-in lifecycle publication. |
| Packaged Flow actions | **Run Record Health Check Rule** and **Run Record Health Check Set**; same underlying logic. |
| Lifecycle events | Opt-in Publish After Commit events for deliberate LWC, Apex, and Flow runs; automatic page-load never publishes. |
...
| Logger | Central log handler (`RecordHealthCheckLogger`): all framework log lines flow here with `runId` correlation and running-user attribution. |
...
> [!NOTE]
> Field reference (Setup labels, API names, picklist values): [Check Set fields](../metadata/fields-check-set.md).
...
> [!NOTE]
> Field reference (Setup labels, API names, per-method fields): [Rule fields](../metadata/fields-rule.md).
...
- If prerequisite is missing, inactive, fails, errors, is skipped, or cannot be evaluated, dependent is `SKIPPED` with `PREREQUISITE_NOT_MET`.
...
| ---------------------------- | -------- |
| `ONE_RESULT` | Expects one row or one aggregate result. |
| `ANY_ROW_PASSES` | Passes when any primary row matches. |
| `ALL_ROWS_PASS` | Passes when every primary row matches. |
| `COMPARE_AS_LISTS` | Full result treated as a list for list operators. |

**List membership exception (`LIST_CONTAINS_ANY` / `LIST_CONTAINS_NONE`):** the primary single value comes from `FindInListFormula__c` (a formula on the record resolving to the value to test), and the comparison list comes from `ComparisonQuery__c` / `ComparisonQueryField__c`. `SourceQuery__c` is not used, and `PassConditionFormula__c` is never read for Query checks. A blank `FindInListFormula__c` yields `INVALID_FORMULA`.
...
| Mode | operators |
| ---- | ----------- |
| `ONE_RESULT` | single-value operators (`EQUALS`, `GREATER_THAN`, `CONTAINS`, and so on). |
| `COMPARE_AS_LISTS` | `LISTS_OVERLAP`, `LISTS_CONTAIN_ALL`, `LISTS_MATCH_EXACTLY`. |
...
Setup label: **Operator**. API values below; Setup picklist labels differ (for example `EQUALS` → **Equals**, `CONTAINS` → **Contains text**, `IS_BLANK` → **Is empty**).

| Operator (API) | Meaning |
| ---------- | ------- |
| `EQUALS` | Primary equals comparison value. |
| `NOT_EQUALS` | Primary does not equal comparison value. |
| `GREATER_THAN` | Primary is greater than comparison value. |
| `GREATER_THAN_OR_EQUAL` | Primary is greater than or equal to comparison value. |
| `LESS_THAN` | Primary is less than comparison value. |
| `LESS_THAN_OR_EQUAL` | Primary is less than or equal to comparison value. |
| `CONTAINS` | Primary text contains comparison text (**case-sensitive**). |
| `DOES_NOT_CONTAIN` | Primary text does not contain comparison text (**case-sensitive**). |
| `IS_BLANK` | Primary value is blank. |
| `IS_NOT_BLANK` | Primary value is not blank. |
| `LIST_CONTAINS_ANY` | Comparison list contains the primary single value. |
| `LIST_CONTAINS_NONE` | Comparison list does not contain the primary single value. |
| `LISTS_OVERLAP` | Two lists share at least one value (case-insensitive). |
| `LISTS_CONTAIN_ALL` | Comparison list contains every value from the primary list (case-insensitive). |
| `LISTS_MATCH_EXACTLY` | Two lists contain the same values and duplicate counts (case-insensitive). |
...
| Mode | Contract |
| ---- | -------- |
| `ALL_RECORDS` | Rule proceeds to evaluation. |
...
| `reasonCode` | Machine-readable reason for skipped, unable, or error results. See [Reason Codes](reason-codes.md). |
...
| `adminDetail` | Structured diagnostics (`RecordHealthCheckAdminDetail`: `containsRestrictedDetail`, `reasonCode`, `message`, `fieldNames`) when Show Diagnostics and View Details authorize it; otherwise null. |
...
| Display policy metadata | Check Set **Found/Expected Display** (`FoundExpectedDisplay__c`): **On demand** (`ON_DEMAND`, default), **Failed checks only** (`FAILURES_ONLY`), or **Every check** (`ALL_ROWS`). Returned to the LWC as `comparisonDisplay` on the definition response. |
| UI visibility (values) | Governed by **Found/Expected Display** and check outcome. **Failures:** Found/Expected inline in every mode when captured. **Passes:** inline only with **Every check** (`ALL_ROWS`); otherwise behind the expander with **On demand** (`ON_DEMAND`) or hidden with **Failed checks only** (`FAILURES_ONLY`). |
...
| Disclosure caret | Shown when values would be revealed on expand. Hidden on passing checks when mode is `FAILURES_ONLY`. `aria-expanded` on the caret; label toggles **Show comparison detail** / **Hide comparison detail**. |
...
| **Troubleshooting** (`adminDetailMessage`, diagnostics-meta) | Engine internals: Reason Codes, timings, Evaluation Type, and raw exceptions. | Diagnostic |
...
**Current behavior:** The **Passes when** line is Advanced-tier only. Business users see the failure message and any display-formula Found/Expected chips; diagnostic details renders in browser console diagnostics only (and only with `ShowDiagnostics__c` on), not on the card.
...
| `comparisonDisplay` | `ON_DEMAND`, `FAILURES_ONLY`, or `ALL_ROWS` from `FoundExpectedDisplay__c`. |
...
Integrations must key on `reasonCode`, never display text. Codes are additive `UPPER_SNAKE_CASE`.

**Remapping:** `FIELD_NOT_ACCESSIBLE` and `RECORD_NOT_ACCESSIBLE` never appear as the public `reasonCode`. They become `CANNOT_EVALUATE`; the specific code is available on `adminDetail.reasonCode` only when Show Diagnostics and View Details are authorized.

**Prerequisites:** public skip code is `PREREQUISITE_NOT_MET` (not the log event name `DEPENDENCY_NOT_PASSED`).

**Applicability gates:** `NOT_APPLICABLE_BY_FORMULA` / `NOT_APPLICABLE_BY_COUNT`. Empty-query skip via `NoRowsResult__c = SKIP` uses `APPLICABILITY_NOT_MET`.

**Token / template failures:** `LEGACY_FLAT_TOKEN`, `UNSUPPORTED_TOKEN_NAMESPACE`, `UNKNOWN_TOKEN_PROPERTY`, `TOKEN_NOT_ALLOWED_ON_SURFACE`, `TOKEN_NOT_AVAILABLE_IN_PHASE`, `MALFORMED_TOKEN`, `TOKEN_LIMIT_EXCEEDED`, `RESOLVED_TEMPLATE_TOO_LONG`.

**LWC-only:** `DEPENDENCY_NOT_IN_RUN` (prerequisite omitted by the 25-Rule cap), plus card setup codes such as `SETUP_REQUIRED`, `NO_ACTIVE_CHECK_SETS`, `INACTIVE_CHECK_SETS_ONLY`, `LOAD_FAILED`.
...
Canonical how-to: [Apex API](../integration/apex-api/public-api.md) and [Flow actions](../integration/flow-actions.md). Lifecycle
publication: [Platform events](../integration/lifecycle-events.md).

The Apex entry point delegates to the same engine as the LWC. Catchable evaluation failures surface as result statuses; uncatchable Apex governor limit exceptions behave like any other Apex API. Sync responses carry the stable `contractVersion = 1.0`.
...
Explicit LWC Run/Rerun, Apex, and Flow runs may publish opt-in platform events after commit when
`PublishRunEvent__c` / `PublishResultEvent__c` are enabled. Automatic LWC page-load evaluations
never publish. See [Lifecycle events](../integration/lifecycle-events.md).

### Anonymous Apex runner

`scripts/apex/runHealthCheck.apex` loads a Check Set definition and evaluates every Rule in priority order for local debugging. Prefer `RecordHealthCheck.runSet` in production callers.
## 14. Logging and observability

All framework log lines flow through `RecordHealthCheckLogger`: the engine, controller, and evaluators never call `System.debug` directly. The log handler can be swapped in one place (for example, Nebula Logger) without touching other classes.
...
### Client-side diagnostics (Show Diagnostics)

Requires **both** `ShowDiagnostics__c` on the Check Set **and** `Record_Health_Check_View_Details` on the running user (included in Permission Set `Record_Health_Check_Admin`). See [Show Diagnostics guide](../guides/show-diagnostics.md).
...
- Runs automatically for `RUN_ON_LOAD` Check Sets; shows **Run** for `RUN_ON_REQUEST`.
...
| `hasCompletedRunOnce` | A run finishes (including a Check Set with zero Rules) | Definitions reload (`recordId` / `checkSetName` change) | Action button **visible label** (`Run` vs `Rerun`) and busy `title` / `aria-label` while a run is in flight |
...
Before the first Manual run (both `ONE_BY_ONE` and `ALL_AT_ONCE`), shows one line:
...
- Always renders `FAIL` (Critical/`CRITICAL`), Warning/`WARNING`, Info/`INFO`, and `UNABLE_TO_EVALUATE` outcomes as full rows: these are actionable and are never collapsed into the summary bar. Only `PASS` and `SKIPPED` outcomes can be collapsed (via `PassedChecksDisplay__c` / `SkippedChecksDisplay__c`).
...
- Replaces the removed standalone "N rules passed" / "N rules were skipped" footer notes. Rows hidden by `SHOW_COUNT_ONLY` still contribute to their pill counts.
...
Requires `Record_Health_Check_View_Details` plus **Show Diagnostics** on the Check Set. Per-row troubleshooting lines, **Troubleshooting detail** on errors, console footnote, and `[RHC]` browser console summary after run completion. See [Show Diagnostics](../guides/show-diagnostics.md).
...
- Apex classes and interfaces (including `RecordHealthCheck`, `RecordHealthCheckLogger`, `RecordHealthCheckConstants`, `RecordHealthCheckSoqlTemplate`, `RecordHealthCheckValueResolver`, `RecordHealthCheckValueSource`)
- Lightning Web Component
- Custom Metadata Type definitions and fields
- No sample Custom Metadata. The local [examples library](../examples/README.md) documents reusable
  Rule patterns.
...
| ----- | ----------- | -------------------------- |
| `EmptyValueHandling__c` | `AS_NO_MATCH` | Blank/invalid falls back to the CMT default for row comparisons. |
| `NoRowsResult__c` | *(none)* | No default in metadata — author must choose `PASS`, `FAIL`, `SKIP`, or `UNABLE_TO_EVALUATE`. |
| `FoundExpectedDisplay__c` | `ON_DEMAND` | Same: unrecognized values fall back to `ON_DEMAND` at runtime. |
...
| ID | Resolution |
| -- | ---------- |
| B1 | Blank `EmptyValueHandling__c` aligns with CMT default (`AS_NO_MATCH`). |
...
| B10 | List membership and list-mode overlap operators are case-insensitive (`CONTAINS` / `DOES_NOT_CONTAIN` remain case-sensitive). |
...
| B25 | `PassedChecksDisplay__c` / `SkippedChecksDisplay__c`: `SHOW_COUNT_ONLY` collapses rows from the list but still populates summary pills after run completion. |
...
| B38 | `Record_Health_Check_Configure` Custom Permission ships in the Admin Permission Set; no runtime branch exists today. |
...
| Server dependency cost | Within one Apex transaction the engine caches prerequisite results in a static map keyed by config + record + check. The LWC still evaluates each Rule in its own transaction, so a prerequisite may run twice (once as its own row, once when a dependent calls the server). | Accept cost for safety on the record-page path; direct Apex chains benefit from the cache. |
| Same-transaction re-evaluation | The dependency cache is **cleared after each top-level** `evaluate()` / `runRule()`. A second call in the same Apex transaction reloads the record and re-evaluates. Caching applies only **within** one evaluation when resolving `PrerequisiteRule__c` chains. | Safe for Flow/batch loops that call `runRule()` after DML; do not rely on the cache carrying over between calls. |
...
| Dual-query list null semantics | Under `AS_NO_MATCH`, each null list value is tracked separately internally, so nulls never match each other. All-null rows with `SKIP_RECORD` → **SKIPPED**. CompareAsLists still applies `NoRowsResult__c` when either side has **zero rows**, without distinguishing which side was empty. | Use applicability SOQL when only one side being empty should change the outcome. |
...
| Schema describe churn | Field planning and validation call `getDescribe()` on hot paths. The Framework caches global/object/field describe per Apex transaction in `RecordHealthCheckDescribeCache`. | No action for normal record-page use; avoid unbounded custom plugins that bypass the cache. |
