...
Both the value and the threshold are on the record, so a Check fields on this record resolves everything in one expression. Found/Expected are **display-only**: they do not change pass/fail.

| Alternative | How it compares | Fit for this check |
| ----------- | --------------- | ------------------ |
| Check fields on this record + Found/Expected | `NumberOfEmployees >= 10` with both sides shown | **This example.** One formula, readable numbers. |
| Check records with a query | `COUNT()` vs FixedValue `10` | Works, but the value is already on the record — no query needed. |
...
| Pass Condition (Formula) | `BLANKVALUE(NumberOfEmployees, 0) >= 10` |
| Found Value Formula | `BLANKVALUE(NumberOfEmployees, 0)` |
| Expected Value Formula | `10` |
| Scalar Formula Return Type | Number |
| Severity | Warning |
| Message When Check Fails | `{!Name} is below the staffing minimum.`<br>*(blank line)*<br>`Compare the Found and Expected values, then update the Employees field.` |

> [!CAUTION]
> **Keep the two display formulas consistent with Pass Condition (Formula).** The engine does **not** compare Found and Expected to each other — `PassFailFormula__c` alone decides pass/fail. If the display formulas describe a different comparison, the row can pass while Found ≠ Expected (or vice versa), which misleads. Mirror each side of the Pass/Fail comparison: here Found is the left operand (`NumberOfEmployees`) and Expected is the right (`10`).

## How it works

```text
-- Pass Condition (Formula) (decides pass/fail; must be Boolean)
...
```text
Pass Condition (Formula):      BLANKVALUE(Debit_Total__c, 0) = BLANKVALUE(Credit_Total__c, 0)
...
Set an Account's Employees to a value below 10 to fail and see **Found** vs **Expected**; set it to 10 or more to pass. Clear the two display formulas to confirm the row falls back to showing only the `Passes when` Pass Condition (Formula) line.
