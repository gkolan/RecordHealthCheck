...
Reach for this pattern when two scalar values from different queries must be compared field-to-field: oldest child value vs parent field, primary contact vs Account attribute, or any paired single-row reads. Use Check fields on this record instead when both fields are reachable via dot notation on one object.
...
| ----------- | --------------- | ------------------ |
| Check fields on this record | `BillingCity = ShippingCity` on Account | Both fields on one row: does not reach oldest Contact by Created Date. |
| Check records with a query | One query returning one Contact | Cannot simultaneously read Billing City from Account in the same One result comparison without a second query. |
...
**Verdict:** Compare two queries is the right evaluator when each side is a single field from a different query. Drop to Check fields on this record when both operands live on the record under test.
...
| Check Type | Compare two queries |
...
| Severity | Info |
| Message When Check Fails | Oldest contact's Mailing City does not match Account Billing City. |
...
Set the component's **Check Set Developer Name** to `Account_Examples_Compare_Two_Queries`. See the [example catalog](../../index.md#sample-check-set-packages) for every Check Set and what it contains.
