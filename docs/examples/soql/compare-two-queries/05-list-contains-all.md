...
| ----------- | --------------- | ------------------ |
| Check fields on this record | | Cannot enumerate Contact cities from two Accounts. |
| Check records with a query | Count child cities | Does not test membership against the parent's city set. |
| Compare two queries | Compare query results as lists + List includes all of | **This example.** Primary list must be contained in comparison list. |
...
| Severity | Warning |
| Message When Check Fails | One or more Contact cities are not present on the parent account. |
