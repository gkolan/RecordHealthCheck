...
| ----------- | --------------- | ------------------ |
| Check fields on this record | | Formula cannot collect Mailing City from Contacts on two Accounts. |
| Check records with a query | One Contact list query | Sees only one Account's cities; parent set is invisible. |
| Compare two queries | Compare query results as lists + Lists share any value | **This example.** Set overlap in metadata. |
...
**Verdict:** Compare two queries with Compare query results as lists is the right evaluator when two query result sets must be compared. Choose Lists share any value for the loosest overlap test; step up operators as the rule tightens.
...
| How To Interpret Query Results | Compare query results as lists |
...
| Severity | Warning |
| Message When Check Fails | No Contact cities are shared with the parent account. |
...
- **Compare query results as lists**: multi-row results become sets before operator evaluation.
