...
| ----------- | --------------- | ------------------ |
| Check fields on this record | | Cannot collect and compare two Contact city sets. |
| Check records with a query | | One query sees only one Account's cities. |
| Compare two queries | Compare query results as lists + Lists match exactly | **This example.** Strictest set equality. |
...
| Severity | Warning |
| Message When Check Fails | Contact cities do not exactly match the parent account: extra or missing values on one side. |
