...
| ----------- | --------------- | ------------------ |
| Check fields on this record | | Formula cannot SUM child Opportunity rows. |
| Check records with a query | `COUNT()` or `AnyRowPasses` on Amount | Tests individual rows, not total pipeline against Annual Revenue. |
...
| Need | Use instead |
| ---- | ----------- |
| Any open Opportunity exists | [Query child count minimum one](../single-query/01-child-count-minimum-one.md) |
| Two plain `COUNT()` values | [Two aggregate counts](01-aggregate-counts.md) |

**Verdict:** Compare two queries earns its place when one side is a child aggregate and the other is a field on the Account (or a second query). Check records with a query fits when the threshold is a fixed literal, not a field value.

## Configuration

| Setup label | Value |
| ----------- | ----- |
| Check Name | Open Sales Pipeline Covers Annual Revenue |
...
| Severity | Warning |
| Message When Check Fails | Total open pipeline is below Annual Revenue. |
