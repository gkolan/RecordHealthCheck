...
Account Rating must be set: non-blank. The Is not empty operator needs no Expected Value Comes From or Expected Value; the operator alone tests population.
...
| Severity | Warning |
| Message When Check Fails | Account Rating has not been set. |
