...
Reach for this pattern when the right-hand side of the comparison should come from a formula on the record under test: percentage of revenue, tier-based minimum, or any per-Account bar. Expected Value Comes From = A formula on this record resolves Expected Formula (on this record) on each evaluation.
...
| ----------- | --------------- | ------------------ |
| Check fields on this record | | Cannot scan child Opportunity rows. |
| Check records with a query | Any row passes + Expected Formula (on this record) | **This example.** Per-Account threshold from parent fields. |
...
**Verdict:** Check records with a query with Expected Value Comes From = A formula on this record is the right evaluator when any child row must clear a bar computed from the Account. Use Compare two queries when comparing two query scalars (for example SUM vs field).
...
| Expected Formula (on this record) | `AnnualRevenue * 0.1` |
| Run This Check When | Always |
| Severity | Warning |
| Message When Check Fails | No open opportunity exceeds 10% of Annual Revenue. |

> [!NOTE]
> This table is the control panel for the check: the single source of truth for every value, so edits here take effect with no code change. Edit Expected Formula (on this record) to change how the per-Account threshold is calculated.

## How it works

For each open Opportunity row, the engine compares Amount to Expected Formula (on this record) evaluated on the Account. Any row that clears the bar passes the check.
...
[← Examples index](../../index.md) · [← Prev: Any row static threshold](05-any-row-static-threshold.md) · [Next: Every record static threshold →](07-all-rows-static-threshold.md)
