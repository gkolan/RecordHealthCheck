...
| ----------- | --------------- | ------------------ |
| Check fields on this record | | Cannot read child Opportunity Amounts. |
| Check records with a query | At least one record must pass + fixed threshold | **This example.** Any-row OR with a static bar. |
...
| Every record must pass | [Every record static threshold](07-all-rows-static-threshold.md) |

**Verdict:** Check records with a query with At least one record must pass and Expected Value Comes From = A fixed value is the right evaluator for "any child exceeds an org-wide threshold." Step to A formula on this record when the bar depends on the Account.
...
| Severity | Warning |
| Message When Check Fails | No open opportunity exceeds $50,000. |
...
- **At least one record must pass**: one qualifying Opportunity is sufficient.
