...
| Check records with a query | `COUNT()` greater than or equal to fixed value | **This example.** Inclusive minimum threshold. |
...
**Verdict:** Check records with a query with greater than or equal and a static Fixed Value is the right evaluator for an inclusive minimum child count. Use greater than with threshold zero when "at least one" is enough.
...
| Severity | Warning |
| Message When Check Fails | This account needs at least two Contacts. |
